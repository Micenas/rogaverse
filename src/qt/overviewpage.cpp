...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizebirjeesAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizebirjeesAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizebirjeesAmount = strAnonymizebirjeesAmount.remove(strAnonymizebirjeesAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizebirjeesAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizebirjeesAmount * COIN) nMaxToAnonymize = nAnonymizebirjeesAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizebirjeesAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizebirjeesAmount));
        strAnonymizebirjeesAmount = strAnonymizebirjeesAmount.remove(strAnonymizebirjeesAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizebirjeesAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
