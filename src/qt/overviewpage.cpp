...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizerogaverseAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizerogaverseAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizerogaverseAmount = strAnonymizerogaverseAmount.remove(strAnonymizerogaverseAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizerogaverseAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizerogaverseAmount * COIN) nMaxToAnonymize = nAnonymizerogaverseAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizerogaverseAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizerogaverseAmount));
        strAnonymizerogaverseAmount = strAnonymizerogaverseAmount.remove(strAnonymizerogaverseAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizerogaverseAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizerogaverseAmount == 0) {
